...
const DashboardAdmin = () => {
  ...
  const cargarConductores = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("https://tourmatch-dw83.onrender.com/api/admin/conductores", {
  ...
  const cargarTuristas = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("https://tourmatch-dw83.onrender.com/api/admin/turistas", {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const cargarReservas = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("https://tourmatch-dw83.onrender.com/api/admin/reservas", {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const ejecutarEliminar = async (id, tipo) => {
    ...
    const url = tipo === 'reserva' ? `https://tourmatch-dw83.onrender.com/api/admin/reservas/${id}` : `https://tourmatch-dw83.onrender.com/api/admin/usuarios/${id}`;
    try {
  ...
  // SOLUCIÓN AL PROBLEMA VISUAL: Extrae de forma correcta los strings desde la fila
  ...
  const ejecutarGuardarEdicion = async (e) => {
    ...
    try {
      const response = await fetch(`https://tourmatch-dw83.onrender.com/api/admin/usuarios/${usuarioAEditar.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        // Sincronización en vivo del listado de React
        if (usuarioAEditar.tipoTab === 'conductores') {
