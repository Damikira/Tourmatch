...
const DashboardAdmin = () => {
  ...
  const cargarConductores = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/admin/conductores", {
  ...
  const cargarTuristas = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/admin/turistas", {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const cargarReservas = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://localhost:8080/api/admin/reservas", {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const ejecutarEliminar = async (id, tipo) => {
    ...
    const url = tipo === 'reserva' ? `http://localhost:8080/api/admin/reservas/${id}` : `http://localhost:8080/api/admin/usuarios/${id}`;
    try {
  ...
  const ejecutarGuardarEdicion = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/admin/usuarios/${usuarioAEditar.id}`, {
        method: 'PUT',
        headers: {
