...
const DashboardConductor = () => {
  ...
  const consultarViajesYActivos = useCallback(async () => {
    ...
    try {
      ...
      const resDisponibles = await fetch(`http://localhost:8080/api/reservas/disponibles?capacity=${capacidadFiltro}`, {
        method: "GET",
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      ...
      const resActivos = await fetch(`http://localhost:8080/api/reservas/mis-viajes-activos`, {
        method: "GET",
        headers: { "Authorization": `Bearer ${user.token}` }
      });
      if (resActivos.ok) {
  ...
  const handleFinalizarViaje = async (id) => {
    if (!user?.token) return;
    try {
      const response = await fetch(`http://localhost:8080/api/reservas/${id}/finalizar`, {
        method: "PUT",
        headers: { "Authorization": `Bearer ${user.token}` }
      });
