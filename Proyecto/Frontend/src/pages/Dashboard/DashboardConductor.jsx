...
const DashboardConductor = () => {
  ...
  const consultarViajesDisponibles = useCallback(async () => {
    ...
    try {
      const response = await fetch(`https://tourmatch-dw83.onrender.com/api/reservas/disponibles?capacidad=4`, {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
  ...
  const consultarViajesActivos = useCallback(async () => {
    if (!user?.token) return;
    try {
      const response = await fetch(`https://tourmatch-dw83.onrender.com/api/reservas/mis-viajes-conductor`, {
        headers: { "Authorization": `Bearer ${user.token}` }
      });
